...
const Profile = () => {
  ...
  // --- NEW: Fetch full user data when component mounts ---
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const response = await api.get('/auth/profile'); // Assuming your profile route is /auth/profile
        // Update context with the full user object received from the backend
  ...
  return (
    // Replaced the entire Bootstrap layout with the custom auth-wrapper
    <div className="auth-wrapper">
      <div className="auth-container">
        
        {/* Profile Content - using a simplified form-container structure */}
        <div className="form-container sign-in-container" style={{width: '100%', position: 'relative'}}>
            <form className="auth-form" style={{padding: '0 40px'}}>
                ...
                >
                    {user.name ? user.name[0].toUpperCase() : <FaUserCircle style={{fontSize: '3rem'}} />}
                </div>
                <h3 className="fw-bold text-dark mb-1">{user.name || 'User'}</h3>
                
                {message && (
                    <div className={`alert alert-${message.type} text-center w-100 my-3`}>
                        {message.text}
                    </div>
                )}
                ...
                <div className="d-flex justify-content-between align-items-center w-100 my-4 pt-2 border-top">
                    <h5 className="mb-0 text-secondary">
                        <FaUserEdit className="me-2 text-primary" size={16} /> User Details
                    </h5>
                    {!isEditing && (
                        <button 
                            type="button"
                ...
                <div className="w-100">
                    ...
                    {/* Full Name Field */}
                    <div className="mb-3">
                        <label className="form-label text-muted small fw-bold w-100 text-start ps-3">Full Name</label>
                        <input 
                            type="text" 
                            className={`auth-input ${isEditing ? '' : 'auth-input-display'}`} // Use auth-input-display for non-editing
                            name="name"
                            value={formData.name}
                    ...
                    <div className="mb-3">
                        ...
                        <input 
                            type="text" 
                            className={`auth-input ${isEditing ? '' : 'auth-input-display'}`}
                            name="phone"
                    ...
                    <div className="mb-3">
                        ...
                        <div className="auth-input-display">
                            {user.email}
                        </div>
                    </div>
                </div>
                ...
                {/* Sign Out Button */}
                <button onClick={logout} className="auth-btn" style={{backgroundColor: '#6c757d', borderColor: '#6c757d'}}>
                    Sign Out
                </button>
            </form>
        </div>
